...
conservacaoTipo$Valor[1] <- sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$`Árvores, animais e água`), na.rm = TRUE) +
  sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$`Tamanho do fragmento`), na.rm = TRUE)

conservacaoTipo$Valor[2] <- sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$Limpeza), na.rm = TRUE) +
  sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$`Respeito às instituições`), na.rm = TRUE) +
  sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$`Engajamento da comunidade e frequência de visitações`), na.rm = TRUE) +
  sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$`Existência de símbolos o sagrado`), na.rm = TRUE) +
  sum(as.numeric(subset(tb, tb$Sistema %in% levels(tb$Sistema)[mata])$Outro), na.rm = TRUE)
...
##*Estado de conservação das Matas*
```{r, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE}
